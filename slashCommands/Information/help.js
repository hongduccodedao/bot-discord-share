...
const getAll = (client, interaction) => {
  ...
  const categories = client.slashCommands
    .map((c) => c.categorySlash)
    .filter((c, i, a) => a.indexOf(c) === i);
  categories.forEach((category) => {
    const commands = client.slashCommands.filter((c) => c.categorySlash === category);
    embed.addFields({
...
const getCommand = (client, interaction) => {
  const command = interaction.options.get("command");
  const commandData = client.slashCommands.find(
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "> Danh mục",
        value: commandData.categorySlash,
        inline: true,
      },
      {
        name: "> Mô tả",
