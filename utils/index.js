module.exports = {
  ...
  getChannel: async function (message, toFind, sameChannelReturn = true) {
    ...
    let channel = await message.guild.channels.resolve(
      toFind.startsWith("<#") ? toFind.slice(2, toFind.length - 1) : toFind
    );
    if (!channel) {
      const listChannel = message.guild.channels.cache
        .filter((c) => c.type == "text")
        .map((ch) => ch.name);
      const matches = findBestMatch(toFind, listChannel);
      if (matches.bestMatch.rating > 0.6)
        channel = message.guild.channels.cache.find(
          (ch) => ch.name == matches.bestMatch.target
        );
    }
